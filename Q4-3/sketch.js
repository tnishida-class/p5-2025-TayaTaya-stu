...
function draw() {
  ...
  for (let i = 0; i < balls.length; i++) {
    let b = balls[i];　　　　　　　　　　　//ｂをボールズ配列のi番目のオブジェクトとする

    if (b.x > 0 && b.x < width && b.y > 0 && b.y < height) { // 画面内
      ballsInCanvas.push(b);　　　　　　　　　　//画面内ならｂオブジェクトをballsincanvasに入れる（仕分け）
    }
  }
  ...
  if (frameCount % 20 === 0) { // 20フレームごとに新しい的を追加する
    ...
    let t = { x: width / 2, y: height / 2, size: 20, vx: random(-5, 5), vy: random(-5, 5) }
    targets.push(t);
  }
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    if (t.size < 200) { // 大きくなりすぎていない
      ...
      for (let j = 0; j < balls.length; j++) { // すべてのボールと衝突判定
        let b = balls[j];
        // BLANK[2]
        hit = dist(b.x, b.y, t.x, t.y) < t.size + b.size
      }
